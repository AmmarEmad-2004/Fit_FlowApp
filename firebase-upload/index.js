...
async function upload() {
  ...
  for (const programType of programTypes) {
    ...
    // Overwrite the program document to clear any old fields.
    await db.collection("programs").doc(programType).set({
      name: programType,
    });
    ...
    for (const dayKey of Object.keys(programData)) {
      ...
      for (let i = 0; i < daysArray.length; i++) {
        ...
        try {
          ...
          console.log(
            `    ✅  day_${i}  │  "${dayData.day}"  │  ${dayData.time} min  │  ${dayData.exercises.length} exercises`
          );
          successCount++;
        } catch (err) {
          console.error(
            `    ❌  day_${i}  │  "${dayData.day}"  │  ERROR: ${err.message}`
          );
          errorCount++;
        }
      }
